...
public class BookModelHelper {
	...
	public static BookModel insertBookModel(SQLiteDatabase db, BookModel book) {
		...
		if (tempBook == null)
			tempBook = BookModelHelper.getBookModel(db, book.getPage(), book.getTitle());
		if (tempBook == null) {
	 ...
	 * @return
	 */
	public static int deleteBookModel(SQLiteDatabase db, BookModel book) {
		int chaptersCount = 0;
		int contentCount = 0;
		ArrayList<PageModel> chapters = book.getChapterCollection();
		if (chapters != null && chapters.size() > 0) {
			for (PageModel chapter : chapters) {
				contentCount += NovelContentModelHelper.deleteNovelContent(db, chapter);
			}
